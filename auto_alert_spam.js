javascript:(function () {
  ...
  function findElementByText(root, selector, texts) {
    root = root || document;
  ...
  function run(startDateStr, endDateStr, waitTimeStr, maxCountStr) {
    ...
    function findMoreButton(article) {
      ...
      var btn = article.querySelector('button[data-testid=\'caret\']');
      if (btn) return btn;
      var path = article.querySelector(
        'button svg path[d=\'M3 12c0-1.1.9-2 2-2s2 .9 2 2-.9 2-2 2-2-.9-2-2zm9 2c1.1 0 2-.9 2-2s-.9-2-2-2-2 .9-2 2 .9 2 2 2zm7 0c1.1 0 2-.9 2-2s-.9-2-2-2-2 .9-2 2 .9 2 2 2z\']'
      );
      if (path && path.closest) {
        var b = path.closest('button');
        if (b) return b;
      }
      return null;
    }

    function findReportMenuItem() {
      var texts = ['ポストを報告', 'ポストを報告する', 'ツイートを報告', 'Report post', 'Report Tweet'];
      var el = findElementByText(document, '[role=\'menuitem\']', texts);
      if (el) return el;
      var span = findElementByText(document, 'span,div', texts);
      if (span && span.closest) {
        var parent = span.closest('[role=\'menuitem\'],button,a,div');
    ...
    function findReportDialog() {
      ...
      for (var i = 0; i < dialogs.length; i++) {
        var dlg = dialogs[i];
        if (!document.body.contains(dlg)) continue;
        var h2List = dlg.querySelectorAll('h2');
        for (var j = 0; j < h2List.length; j++) {
          var t = (h2List[j].innerText || h2List[j].textContent || '').trim();
          if (!t) continue;
          if (t.indexOf('何を報告していますか？') !== -1 || t.indexOf('What do you want to report?') !== -1) {
            return dlg;
          }
        }
      }
    ...
    function findHateOption(root) {
      root = root || document;
      var texts = ['ヘイト、攻撃的な行為、または嫌がらせ', 'ヘイト、攻撃的な行為'];
      var label = findElementByText(root, 'label', texts);
      if (label) return label;
      var option = findElementByText(root, '[role=\'radio\'],[role=\'option\']', texts);
    ...
    function findNextButton(root) {
      root = root || document;
      var btn = root.querySelector('button[data-testid=\'ChoiceSelectionNextButton\']');
      if (btn) return btn;
      var texts = ['次へ', 'Next'];
      return findElementByText(root, 'button', texts);
    }
    ...
    function findCompleteButton(root) {
      root = root || document;
      var byTestId = root.querySelector('[data-testid=\'ocfSettingsListNextButton\']');
      if (byTestId && isVisible(byTestId)) return byTestId;
      var texts = ['完了', 'Done', '送信', 'Submit'];
      var btn = findElementByText(root, 'button', texts);
      if (btn && isVisible(btn)) return btn;
      var el = findElementByText(root, 'span,div', texts);
      if (el) {
        var clickable = el.closest('button') || el.closest('[role=\'button\']');
        if (clickable && isVisible(clickable)) return clickable;
        var outer = el.closest('[dir=\'ltr\']');
        if (outer && isVisible(outer)) return outer;
        if (isVisible(el)) return el;
        return el.parentElement && isVisible(el.parentElement) ? el.parentElement : null;
      }
      return null;
    }

    function processTweet(article, onDone) {
      ...
      var moreBtn = findMoreButton(article);
      if (!moreBtn) {
        onDone(false);
        return;
      }
      ...
      var menuAttempts = 0;
      function tryClickReport() {
        menuAttempts++;
        var reportItem = findReportMenuItem();
        if (reportItem) {
          try {
            reportItem.click();
          } catch (err) {
          ...
          setTimeout(handleReportDialog, 700);
          return;
        }
        if (menuAttempts > 15) {
          onDone(false);
      ...
      function handleReportDialog() {
        ...
        function tryHandle() {
          ...
          var hateOption = findHateOption(root);
          if (hateOption) {
            try {
              hateOption.click();
            } catch (err) {}
            setTimeout(function () {
              var nextBtn = findNextButton(root);
      ...
      function tryClickComplete() {
        var completeAttempts = 0;
        function attempt() {
          completeAttempts++;
          var completeBtn = findCompleteButton(document);
          if (completeBtn && !completeBtn.disabled && completeBtn.getAttribute('aria-disabled') !== 'true') {
            try {
              completeBtn.click();
            } catch (err) {}
            setTimeout(function () {
    ...
    stepFn = function step() {
      ...
      try {
        ...
        if (targetTweet) {
          ...
          if (canScrollWindow) {
            try {
              updateProgress(progressDiv, successCount, totalDisplayFn(), '次のポストを探しています...');
            } catch (err) {}
            ...
            var totalDisplay2 = totalDisplayFn();
            setTimeout(function () {
              if (reloadRetryTimeout) return;
              if (noTargetRetryCount < NO_TARGET_RETRY_MAX) {
                ...
                try {
                  updateProgress(
                    ...
                    successCount,
                    totalDisplay2,
                    'ターゲットなし: 再試行 ' + noTargetRetryCount + '/' + NO_TARGET_RETRY_MAX + '...'
                  );
                } catch (err) {}
                scheduleRetry();
                return;
  ...
  (function (callback) {
    ...
    var btnStart = dlg.querySelector('#xAlertConfirmBtn');
